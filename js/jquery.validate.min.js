...
(function (c) {
    c.extend(c.fn, {
        validate: function (a) {
            if (this.length) {
                var b = c.data(this[0], "validator"); if (b) return b; if (typeof (Worker) !== "undefined") { this.attr('novalidate', 'novalidate'); }; b = new c.validator(a, this[0]); c.data(this[0], "validator", b); if (b.settings.onsubmit) {
        ...
        b) { this.settings = c.extend(true, {}, c.validator.defaults, a); this.currentForm = b; this.init() }; c.validator.format = function (a, b) { if (arguments.length == 1) return function () { var d = c.makeArray(arguments); d.unshift(a); return c.validator.format.apply(this, d) }; if (arguments.length > 2 && b.constructor != Array) b = c.makeArray(arguments).slice(1); if (b.constructor != Array) b = [b]; c.each(b, function (d, e) { a = a.replace(RegExp("\\{" + d + "\\}", "g"), e) }); return a }; c.extend(c.validator, {
            defaults: {
                ...
                required: "必选字段。", remote: "请修正该字段。", email: "请输入正确格式的电子邮件。", url: "请输入合法的网址。", date: "请输入合法的日期。", dateISO: "请输入合法的日期 (ISO)。", number: "请输入合法的数字。", digits: "只能输入整数。", creditcard: "请输入合法的信用卡号。", equalTo: "请再次输入相同的值。", accept: "请输入拥有合法后缀名的字符串。", maxlength: c.validator.format("请输入一个长度最多是 {0} 的字符串。"),
                minlength: c.validator.format("请输入一个长度最少是 {0} 的字符串。"), rangelength: c.validator.format("请输入一个长度介于 {0} 和 {1} 之间的字符串。"), range: c.validator.format("请输入一个介于 {0} 和 {1} 之间的值。"), max: c.validator.format("请输入一个最大为 {0} 的值。"), min: c.validator.format("请输入一个最小为 {0} 的值。")
            }, autoCreateRanges: false, prototype: {
                init: function () {
                    function a(e) {
                        var f = c.data(this[0].form, "validator"), g = "on" + e.type.replace(/^validate/,
                            ""); if (!f) return; f.settings[g] && f.settings[g].call(f, this[0], e)
